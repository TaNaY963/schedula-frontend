...
function visitTypeLabel(type: Appointment["type"]) {
  return type === "in-person" ? "In person" : "Video";
}
...
export default function Home() {
  ...
  const counts = items.reduce<Record<Filter, number>>(
    (total, item) => ({
      ...total,
      all: total.all + 1,
                      ...
                      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          <p className="font-semibold">
                            {item.patientName}{" "}
                            <span className="font-normal text-[var(--muted)]">· {durationMinutes(item.startTime, item.endTime)} min</span>
                          </p>
                          <p className="mt-0.5 truncate text-sm text-[var(--muted)]">
                            {item.reason} · {item.doctorName}
                          </p>
                        </div>
              ...
              <div className="mt-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[var(--muted)]">{visitTypeLabel(selected.type)}</p>
                  </div>
                </div>
                <dl className="mt-6 space-y-4 text-sm">
                  ...
                  <div>
                    <dt className="text-[var(--muted)]">Time & type</dt>
                    <dd className="mt-1 font-medium">
                      {formatTime(selected.startTime)} · {visitTypeLabel(selected.type)}
                    </dd>
                  </div>
                  <div>
